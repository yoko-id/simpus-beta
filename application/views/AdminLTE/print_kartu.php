<?php $this->load->view('AdminLTE/header'); ?>
<?php $this->load->view('AdminLTE/sidebar'); ?>

  <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    <!-- Main content -->
    <section class="content">
		<!-- Small boxes (Stat box) -->		
		<div class="row">
			<div class="col-lg-12 col-xs-12">
				<div class="box box-info">
					<div class="box-header with-border">
						<h3 class="box-title">Daftar Pasien</h3>
						<div class="pull-right">
							
						</div>
					</div>
					
					<div class="box-body">
						<form method="post" target="_blank">
							<div class="btn-group clearfix" role="group" style="margin-bottom: 10px;">
								<button type="submit" class="btn btn-default" id="print"><i class="glyphicon glyphicon-print"></i> Cetak Kartu Pasien</button>
							</div>
							
							<table class="table table-striped table-bordered" cellspacing="0" width="100%">
								<thead>
									<tr>
										<th width="10%"><input type="checkbox" id="select_all"></th>
										<th width="10%">NO INDEX</th>
										<th>NAMA PASIEN</th>
										<th width="5%">UMUR</th>
										<th width="5%">KELAMIN</th>
										<th>ALAMAT</th>
										<th>NO BPJS</th>
									</tr>
								</thead>
								<tbody>
								<?php
								foreach($result as $row){
								?>
									<tr>
										<td><input type="checkbox" name="nokartu[]" value="<?=$row->no_index;?>"></td>
										<td><?=$row->no_index;?></td>
										<td><?=$row->nama_pasien;?></td>
										<td><?=get_age($row->tgl_lahir);?> Th</td>
										<td><?=$row->gender;?></td>
										<td><?=$row->desa;?></td>
										<td><?=$row->no_bpjs;?></td>
									</tr>
								<?php
								}
								?>
								</tbody>
							</table>
							
							<div class="btn-group clearfix" role="group" style="margin-top: 10px;">
								<button type="submit" class="btn btn-default" id="print"><i class="glyphicon glyphicon-print"></i> Cetak Kartu Pasien</button>
							</div>
						</form>
					</div>
					<!-- /.box-body -->
				</div>
			</div>
		</div>
		<!-- /.row -->
    </section>
    <!-- /.content -->
  </div>
  <!-- /.content-wrapper -->
  
	<script>
	$(document).ready(function(){
		$("button#cetakpdf").click(function(){
			//window.open(base_url + 'laporan/pasien/pdf', '_blank');
		});
		$("button#cetakcsv").click(function(){
			window.open(base_url + 'laporan/pasien/xls', '_blank');
		});
		
		var maxallowed = 10;
		$('input#select_all').click(function() {
			var c = $("input[name='select']:checked").length;
			for(i=0; i<=maxallowed; i++){
				$(':checkbox').prop('checked',c);
			}
		});
		
		$("input[name='select']").change(function () {
			var cnt = $("input[name='select']:checked").length;
			if (cnt > maxallowed){
				$(this).prop("checked", "");
				alert('Select maximum ' + maxallowed + '!');
			}
		});
	});
	</script>
	  
	  
  
<?php $this->load->view('AdminLTE/footer'); ?>